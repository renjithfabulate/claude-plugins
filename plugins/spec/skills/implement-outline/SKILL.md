...
Read the latest `structure-outline`. Work from it directly: **never** call `/spec:create-plan` or
`/spec:implement-plan` from here.
...
If every phase is done, say so and suggest `/spec:describe-pr <TICKET-ID>`.
