...
function resolveKey() {
  ...
  // 'spec' is the current name; 'rpi' is the pre-rename path, still honoured so an
  // existing key keeps working without the user having to move the file.
  for (const dir of ['spec', 'rpi']) {
    const cfg = join(homedir(), '.config', dir, 'linear.env');
