...
argument-hint: [TASK-ID] [what is wrong]
...
- **A phase that was never started** is not iteration. Send it to `/spec:implement-outline` or
  `/spec:implement-plan`, which is where branch handling, checks and per-phase commits live.
- **Behaviour that is wrong because the design was wrong** is a design change. Say so and route to
  `/spec:iterate-design-discussion` or `/spec:iterate-tdd`. Fixing it here leaves the artifacts
