...
Writes `.spec/config.json` **in the current repo**. The plugin itself ships no project-specific
...
Read `.spec/config.json` if it already exists. Look for the package manager in use (`package.json`,
...
`.spec/config.json` is shared team configuration, so it belongs in the repo. It holds no secrets: the
